...
public class BoardManager extends AdminManager {
	...
	private SqlMethodBucket sqlMethodBucket = null;
	...
	public BoardManager(SqlMethodBucket sqlMethodBucket) {
		super();
		this.sqlMethodBucket = sqlMethodBucket;
	}
	
	public static BoardManager getInstance(SqlMethodBucket sqlMethodBucket) {
		if(boardManager == null)
			boardManager = new BoardManager(sqlMethodBucket);
		return boardManager;
	}
	...
	public int getMaxNum(String boardName) throws SQLException {
		...
		return sqlMethodBucket.executeQueryNum(sql);
	}
	...
	public BoardData getNotice(String boardName, int num) throws SQLException {
		...
		Vector<BoardData> vector = sqlMethodBucket.executeQuery(sql);
	...
	public void replyNotice(String boardName, BoardData data) throws SQLException {
		...
		int mstep = sqlMethodBucket.executeQueryNum(sql);
		int instep = 0;
		...
		if(mstep > 0) {	// 답변 글이 중간에 끼어들어갈 경우.
			...
			instep = sqlMethodBucket.executeQueryNum(sql) + 1;
		}
		...
		for(int r = depth - 1; r >= 0; r--) {
			sql = "SELECT max(step) FROM " + boardName + " WHERE ref=" + data.getRef() +
																	" and step<" + mstep + 
																	" and depth=" + r;
			int maxStep = sqlMethodBucket.executeQueryNum(sql);
	...
	public Vector<BoardData> getPage(String boardName, int pageNumber) throws SQLException {
		...
		return sqlMethodBucket.executeQuery(sql);
	}
	...
	public int[] getNextPrevNum(String boardName, int num) throws SQLException {
		...
		nums[0] = sqlMethodBucket.executeQueryNum(sql);
		
		sql = "SELECT num FROM " + boardName + " WHERE num<" + num;
		nums[1] = sqlMethodBucket.executeQueryNum(sql);
	...
	public int getRecNum(String boardName) throws SQLException {
		...
		recordNum = sqlMethodBucket.executeQueryNum(sql);
		return recordNum;
	}
}
